// Captura do Decision Card em spots de RIVER cobertos (estratégia do solver).
import { chromium } from 'playwright';
import fs from 'fs';
const TOKEN = process.env.LL_TOKEN;
const OUT = 'C:/Users/rodri/AppData/Local/Temp/replayer_shots';
fs.mkdirSync(OUT, { recursive: true });
// [tag, t, h, heroAction] — pega o ÚLTIMO card solver cuja ação bate (river é a última)
const SPOTS = [
  ['river_critical', '3910307458', '257046503651', 'check'],  // river check vs GTO allin = gto_critical
  ['river_correct',  '3910307458', '257045919085', 'check'],  // river check/check = gto_correct
  ['river_mixed',    '3910307458', '257045965983', 'bet'],    // river bet vs check = gto_mixed
];
...
for (const [tag, T, H, act] of SPOTS) {
  ...
  else console.log(`  [${tag}] NENHUM card river encontrado`);
  await ctx.close();
}
await browser.close();
console.log('DONE');
