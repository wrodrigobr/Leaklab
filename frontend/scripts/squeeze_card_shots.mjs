// Captura do Decision Card em spots de SQUEEZE (hero squeezando) — torneio sintético.
...
const SPOTS = [
  ['sqz_aks', 'SQZSWEEP1', 'SQZ001'],  // AKs shove = correct
  ['sqz_aa',  'SQZSWEEP1', 'SQZ002'],  // AA shove = major_leak (deve raise)
  ['sqz_kjo', 'SQZSWEEP1', 'SQZ003'],  // KJo shove = major_leak (fold)
];
...
for (const [tag, T, H] of SPOTS) {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    if (hasPlayed && txt.includes('squeeze')) {
  ...
  if (!shot) console.log(`  [${tag}] NENHUM card squeeze encontrado`);
  await ctx.close();
}
await browser.close();
console.log('DONE');
