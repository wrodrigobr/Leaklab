...
export default function AcademyMath() {
  ...
  const L = (k: string) => t(`lessons.math.${k}`);
  const you = L("seat_you"), vil = L("seat_vil");

  const loadDrill   = useCallback(() => academy.mathQuestion("beginner"), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.mathSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Cena — projeto de cor: você tem A-K de copas num flop com duas copas, enfrentando aposta.
  const drawStep = scene({
    type: "action", seat: 2, player: you, is_hero: true,
    seats: {
      "1": { player: vil, stack: 40, stack_bb: 40, pos: "CO" },
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="max-w-4xl space-y-12 pb-8">
        ...
        <LessonSection n={4} title={L("c2_title")}>
          <Prose html={L("c2_p1")} />
          <TableScene step={drawStep} hero={you} heroCards={["Ah", "Kh"]} caption={L("c2_cap")} />
          <Prose html={L("c2_p2")} />
        ...
        <section className="space-y-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <QuizRunner theme="emerald" Icon={Calculator} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
