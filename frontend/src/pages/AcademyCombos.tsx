...
import { Hash, Dumbbell } from "lucide-react";
...
export default function AcademyCombos() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.combos.${k}`);

  const loadDrill   = useCallback(() => academy.combosQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.combosSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="max-w-4xl space-y-12 pb-8">
        ...
        <section className="space-y-4 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          <QuizRunner theme="amber" Icon={Hash} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
        ...
        <NextStep to="/academy/mdf" label={L("next_label")} sub={L("next_sub")} />
      </article>
    </HudLayout>
  );
}
