...
export default function AcademyPushFold() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.pushfold.${k}`);
  const you = L("seat_you"), villain = L("seat_villain"), other = L("seat_other");

  const loadDrill   = useCallback(() => academy.pushfoldQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.pushfoldSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Cena 1 — botão curto (6bb), fold até você: shove largo é correto (muita fold equity).
  const shoveStep = scene({
    type: "action", street: "preflop", seat: 2, player: you, is_hero: true,
    seats: {
      "1": { player: other,   stack: 22, stack_bb: 22, pos: "CO" },
      "2": { player: you,     stack: 6,  stack_bb: 6,  pos: "BTN" },
      "3": { player: villain, stack: 18, stack_bb: 18, pos: "BB" },
    },
    hero: you, hero_cards: ["Ad", "9s"], board: [],
    pot: 1.5, pot_bb: 1.5, button: 2,
  });

  // Cena 2 — UTG curto (7bb): mesma profundidade, mas com a mesa toda atrás, mão marginal FOLDA.
  const foldStep = scene({
    type: "action", street: "preflop", seat: 1, player: you, is_hero: true,
    seats: {
      "1": { player: you,     stack: 7,  stack_bb: 7,  pos: "UTG" },
      "2": { player: other,   stack: 30, stack_bb: 30, pos: "CO" },
      "3": { player: villain, stack: 25, stack_bb: 25, pos: "BTN" },
    },
    hero: you, hero_cards: ["Kd", "9c"], board: [],
    pot: 1.5, pot_bb: 1.5, button: 3,
  });
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="mx-auto max-w-2xl space-y-12 pb-8">
        ...
        <LessonSection n={3} title={L("c1_title")}>
          <Prose html={L("c1_p1")} />
          <TableScene step={shoveStep} hero={you} heroCards={["Ad", "9s"]} caption={L("c1_cap")} />
        ...
        <LessonSection n={4} title={L("c2_title")}>
          ...
          />
          <Prose html={L("c2_p2")} />
          <TableScene step={foldStep} hero={you} heroCards={["Kd", "9c"]} caption={L("c2_cap")} />
        ...
        <section className="space-y-4 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          <QuizRunner theme="amber" Icon={Zap} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
