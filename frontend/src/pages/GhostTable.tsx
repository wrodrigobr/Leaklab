...
// A conversao spot -> step da mesa mora em `lib/mesaDoDrill`: o modo Pratica (1 a 4 mesas)
// usa a MESMA, e duas copias divergindo mostrariam a mesa certa num treino e a errada no outro.
import { parseCards as _parseCards, buildDrillStep } from "@/lib/mesaDoDrill";
...
export default function GhostTable() {
  ...
  return (
    <HudLayout eyebrow="Ghost Table" title={t("title")} description={t("subtitle")}>
      ...
      {/* ACTIVE/RESULT nao renderizam aqui: o layout full-screen (early return acima,
          `if (current && ...)`) captura essas fases antes de chegar neste return. As copias
          antigas que viviam aqui eram codigo morto e ja tinham divergido (isCallEqualToJam
          nem existia neste escopo) — removidas em vez de mantidas compilando. */}
