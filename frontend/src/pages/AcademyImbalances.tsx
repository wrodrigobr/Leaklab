...
export default function AcademyImbalances() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.imbalances.${k}`);

  const loadDrill   = useCallback(() => academy.imbalancesQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.imbalancesSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );
  ...
  return (
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="mx-auto max-w-2xl space-y-12 pb-8">
        ...
        <Objectives title={L("obj_title")} items={[L("obj1"), L("obj2"), L("obj3"), L("obj4"), L("obj5")]} />
        ...
        <LessonSection n={5} title={L("c3_title")}>
          <Prose html={L("c3_p1")} />
          <Callout tone="key" title={L("c3_key_title")}>{L("c3_key")}</Callout>
        </LessonSection>
        ...
        <section className="space-y-4 rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
          ...
          <QuizRunner theme="violet" Icon={Scale} loadFn={loadDrill} submitFn={submitDrill} />
        </section>

        <Takeaways title={L("takeaways_title")} items={[L("t1"), L("t2"), L("t3"), L("t4"), L("t5")]} />
        ...
        <NextStep to="/academy/exploits" label={L("next_label")} sub={L("next_sub")} />
      </article>
    </HudLayout>
  );
}
