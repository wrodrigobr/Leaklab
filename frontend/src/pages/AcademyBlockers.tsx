...
import { Ban, Dumbbell } from "lucide-react";
...
export default function AcademyBlockers() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.blockers.${k}`);
  const you = L("seat_you"), vil = L("seat_vil");

  const loadDrill   = useCallback(() => academy.blockersQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.blockersSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Cena — river com 3 copas (cor possível). Você tem o Ás de copas: bloqueia a nut flush.
  const blockerStep = scene({
    ...
    hero: you, hero_cards: ["Ah", "5c"], board: ["Qh", "8h", "3h", "2c", "9d"],
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="max-w-6xl space-y-12 pb-8">
        ...
        <LessonSection n={3} title={L("c1_title")}>
          <Prose html={L("c1_p1")} />
          <TableScene step={blockerStep} hero={you} heroCards={["Ah", "5c"]} caption={L("c1_cap")} />
        ...
        <section className="space-y-4 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          <QuizRunner theme="amber" Icon={Ban} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
        ...
        <NextStep to="/academy/combos" label={L("next_label")} sub={L("next_sub")} />
      </article>
    </HudLayout>
  );
}
