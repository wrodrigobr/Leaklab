...
export default function AcademyPostflop() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.postflop.${k}`);
  const you = L("seat_you"), vil = L("seat_vil");

  const loadDrill   = useCallback(() => academy.postflopQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.postflopSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Board SECO — você é o agressor em posição com cartas altas.
  const dryStep = scene({
    seats: {
      "1": { player: vil, stack: 40, stack_bb: 40, pos: "BB" },
      "2": { player: you, stack: 40, stack_bb: 40, pos: "BTN" },
    },
    hero: you, hero_cards: ["Ah", "Qs"], board: ["Ks", "7d", "2c"],
    pot: 6, pot_bb: 6, button: 2,
  });

  // Board MOLHADO — conectado e de dois naipes.
  const wetStep = scene({
    seats: {
      "1": { player: vil, stack: 40, stack_bb: 40, pos: "BB" },
      "2": { player: you, stack: 40, stack_bb: 40, pos: "CO" },
    },
    hero: you, hero_cards: ["Ad", "Ac"], board: ["9h", "8h", "7c"],
    pot: 6, pot_bb: 6, button: 2,
  });
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="max-w-4xl space-y-12 pb-8">
        ...
        <LessonSection n={4} title={L("c2_title")}>
          <Prose html={L("c2_p1")} />
          <TableScene step={dryStep} hero={you} heroCards={["Ah", "Qs"]} caption={L("c2_cap_dry")} />
          <Prose html={L("c2_p2")} />
          <TableScene step={wetStep} hero={you} heroCards={["Ad", "Ac"]} caption={L("c2_cap_wet")} />
        </LessonSection>
        ...
        <LessonSection n={7} title={L("c5_title")}>
          <Prose html={L("c5_p1")} />
          <Callout tone="warn" title={L("c5_warn_title")}>{L("c5_warn")}</Callout>
        </LessonSection>
        ...
        <section className="space-y-4 rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
          ...
          <QuizRunner theme="violet" Icon={Layers} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
