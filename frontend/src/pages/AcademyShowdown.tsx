...
export default function AcademyShowdown() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.showdown.${k}`);
  const you = L("seat_you"), vil = L("seat_vil");

  const loadDrill   = useCallback(() => academy.showdownQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.showdownSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Cena — par de 7 no river: ganha de blefes, perde de valor. Mão de showdown value.
  const sdvStep = scene({
    ...
    hero: you, hero_cards: ["7h", "7s"], board: ["Ks", "9d", "4c", "2h", "Jc"],
    pot: 12, pot_bb: 12, button: 2,
  });
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="mx-auto max-w-2xl space-y-12 pb-8">
        ...
        <LessonSection n={3} title={L("c1_title")}>
          <Prose html={L("c1_p1")} />
          <TableScene step={sdvStep} hero={you} heroCards={["7h", "7s"]} caption={L("c1_cap")} />
        ...
        <section className="space-y-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <QuizRunner theme="emerald" Icon={Eye} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
