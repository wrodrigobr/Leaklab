...
export default function AcademyPosition() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.position.${k}`);
  const you = L("seat_you"), vA = L("seat_vA"), vB = L("seat_vB");

  const loadDrill   = useCallback(() => academy.positionQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.positionSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Cena — você no BTN (o melhor assento): age por último no pós-flop.
  const btnStep = scene({
    type: "action", street: "preflop", seat: 2, player: you, is_hero: true,
    seats: {
      "1": { player: vA, stack: 40, stack_bb: 40, pos: "CO" },
      "2": { player: you, stack: 40, stack_bb: 40, pos: "BTN" },
      "3": { player: vB, stack: 40, stack_bb: 40, pos: "BB" },
    },
    hero: you, hero_cards: ["Ah", "Kd"], board: [],
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="mx-auto max-w-2xl space-y-12 pb-8">
        ...
        <LessonSection n={3} title={L("c1_title")}>
          <Prose html={L("c1_p1")} />
          <TableScene step={btnStep} hero={you} heroCards={["Ah", "Kd"]} caption={L("c1_cap")} />
        ...
        <section className="space-y-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <QuizRunner theme="emerald" Icon={Target} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
        ...
        <NextStep to="/academy/math" label={L("next_label")} sub={L("next_sub")} />
      </article>
    </HudLayout>
  );
}
