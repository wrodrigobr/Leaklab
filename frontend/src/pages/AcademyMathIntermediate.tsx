...
import { Sigma, Dumbbell } from "lucide-react";
...
export default function AcademyMathIntermediate() {
  ...
  const L = (k: string) => t(`lessons.math_int.${k}`);
  const you = L("seat_you"), vil = L("seat_vil");

  const loadDrill   = useCallback(() => academy.mathQuestion("intermediate"), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.mathSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );

  // Cena — projeto combinado: J-T de copas num flop 9h-8s-2h (cor + sequência aberta).
  const comboStep = scene({
    ...
    hero: you, hero_cards: ["Jh", "Th"], board: ["9h", "8s", "2h"],
    bets: { [vil]: 5 }, pot: 15, pot_bb: 15, button: 2,
  });
    ...
    <HudLayout eyebrow={L("eyebrow")} title={L("title")} description={L("subtitle")}>
      <article className="mx-auto max-w-2xl space-y-12 pb-8">
        ...
        <LessonSection n={4} title={L("c2_title")}>
          <Prose html={L("c2_p1")} />
          <TableScene step={comboStep} hero={you} heroCards={["Jh", "Th"]} caption={L("c2_cap")} />
        ...
        <section className="space-y-4 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
          ...
          <QuizRunner theme="amber" Icon={Sigma} loadFn={loadDrill} submitFn={submitDrill} />
        </section>
