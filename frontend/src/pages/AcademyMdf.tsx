...
export default function AcademyMdf() {
  const { t } = useTranslation("academy");
  const L = (k: string) => t(`lessons.mdf.${k}`);

  const loadDrill   = useCallback(() => academy.mdfQuestion(), []);
  const submitDrill = useCallback(
    (idx: number, ci: number, xp: number) => { academy.mdfSubmit(idx, ci, xp).catch(() => {}); },
    [],
  );
