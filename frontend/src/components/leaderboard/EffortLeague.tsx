    ...
    <div className="rounded-xl border border-primary/40 bg-primary/[0.06] p-4">
      ...
      <div className="space-y-1">
        ...
        <div className="font-mono text-[11px] text-muted-foreground">
          {me.points} {t("league.points")} · {me.spots} {t("league.spots")}
        </div>
        <div className="text-[11px] text-muted-foreground">{t("league.public")}</div>
      </div>
    </div>
  );
}
...
/** Linha de jogador na liga de esforço. MESMA estrutura do eixo skill: cabeçalho
 *  (rank + nome/streak + acertos em destaque) + duas barras (acertos + volume). */
function EffortRow({ e, self, maxPoints, maxSpots }:
  { e: TrainingLeagueEntry; self?: boolean; maxPoints: number; maxSpots: number }) {
  const { t } = useTranslation("training");
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-1 gap-y-1.5">
        <EffortBar label={t("league.points")} pct={maxPoints > 0 ? (e.points / maxPoints) * 100 : 0} />
        <EffortBar label={t("league.spots")} pct={maxSpots > 0 ? (e.spots / maxSpots) * 100 : 0} />
      </div>
    </div>
  );
}
...
export function EffortLeague() {
  ...
  const maxSpots = Math.max(1, ...ranked.map((r) => r.spots));
            ...
            <div className="space-y-2">
              {ranked.map((e) => (
                <EffortRow
                  key={e.user_id}
                  e={e}
                  self={!!me?.is_self && e.user_id === me.user_id}
                  maxPoints={maxPoints}
                  maxSpots={maxSpots}
                />
              ))}
