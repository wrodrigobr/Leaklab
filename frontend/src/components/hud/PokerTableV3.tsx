...
function renderSeatsAndChips(
  ...
  seatNums.forEach((sn) => {
    ...
    if (isBtn) {
      ...
      const perpDist = isHero ? 90 : isAdjRightBtn ? 28 : 38;
    ...
    if (bet > 0) {
      ...
      let t2 = isHero ? 0.46 : isSide ? 0.20 : 0.36;
      if (isAdjT2) t2 = 0.42;
      ...
      if (isHero) {
        ...
        if (isAdjacentToHero) {
          ...
          perpOffX = Math.round(sign * (-dvy / blen) * 14);
          perpOffY = Math.round(sign * (dvx / blen) * 14);
        }
      }
