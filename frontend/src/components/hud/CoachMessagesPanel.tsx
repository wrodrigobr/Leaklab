...
interface Props {
  ...
  /** When true, renders as a full-height panel (no collapsible header). Used inside the drawer. */
  drawer?: boolean;
}

export function CoachMessagesPanel({ coachUsername, drawer = false }: Props) {
  ...
  useEffect(() => {
    if (open || drawer) bottomRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [data?.messages, open, drawer]);

  useEffect(() => {
    if ((open || drawer) && data?.messages?.length) {
      qc.invalidateQueries({ queryKey: ["player-messages-unread"] });
    }
  }, [open, drawer, data?.messages?.length, qc]);
  ...
  const messagesList = (
    <div className={drawer ? "flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0" : "h-64 overflow-y-auto px-4 py-3 space-y-3"}>
      {isLoading && (
  ...
  const inputBar = (
    <div className="border-t border-border px-3 py-2.5 flex gap-2 shrink-0">
      <textarea
        value={body}
