...
### feat(coach-replay): vira o WALKTHROUGH da sessão (playlist filtrada + narração + navegação na mesa real) (#flagship)

> O Coach Replay deixou de ser "top-3 erros com deep-link" e virou o que o produto prometia: **revisar a sessão inteira na mesa real**, pulando o ruído. Backend (`coach_replay.py`) monta a PLAYLIST cronológica (`hand_id` asc) das mãos que valem revisão, com filtro apertado "só decisões reais": mantém a mão se viu o flop, OU tem qualquer erro/desvio (severidade via `verdict.verdict3` sobre `label`, a régua única de display), OU foi confronto all-in pré-flop (shove ou showdown pré-flop); cai fora o grosso dos folds pré-flop corretos e os opens triviais que levaram os blinds. Cada mão traz veredito (erro/aceitável/correto), custo de EV e uma **narração pronta do coach** (frases determinísticas, sem custo de LLM, PT sem travessão; o número de bb fica só no badge pra não divergir do texto). Em torneios reais: SNG 65→29 mãos, MTT 396→106. Frontend: a página `/coach-replay/:code` lista a sessão (cada mão colorida por veredito, com cartas, posição, rua e narração) + "Começar revisão"; e o **Replayer ganha modo walkthrough** (`?walk=1`): restringe a navegação prev/próxima à playlist (reusa `handList` + o progresso N/M já existentes), preserva o `walk` na URL entre mãos (novo helper `handHref`) e mostra uma barra do coach com a narração da mão + "Visão geral". Zero mudança na renderização da mesa: é uma camada por cima do Replayer que já existe.
