...
### feat(hand-builder): "Finalizar aleatoriamente" quando o hero folda

> Depois que o hero folda, o resto da mão **não afeta a análise** (só decisões do hero contam) — então não faz sentido obrigar a completar villains/board/vencedor. Quando o hero registra um fold e a mão não acabou, aparece um aviso "o hero foldou…" + botão **"Finalizar aleatoriamente"**: os villains ativos restantes foldam pra um **vencedor aleatório** e a mão fecha na street atual (sem precisar de board/showdown), pronta pro "Próxima mão". Pure/testável em `lib/hhAutoFinish.ts` (`autoFinishAfterFold` com `rand` injetável; `totalPot`) + `hhAutoFinish.test.ts` (5). **Validado:** round-trip backend de uma mão auto-finalizada → exatamente **1 decisão do hero** (BTN fold vs UTG = `gto_correct`), os folds aleatórios dos villains **não** geram decisões espúrias; e2e (browser real) confirma o botão só aparecer após o hero foldar e a mão resolver com vencedor ("collected" no HH, "BTN: folds" preservado). i18n 3 locales; typecheck + build + vitest 26/26.
