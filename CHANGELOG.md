...
> Com o fallback multiway ativo, o veredito virava Fold mas várias seções ainda liam dados do engine/solver HU e **contradiziam** o fold: o racional "mão forte: raise por valor", a pill "Sua mão 37% marginal" (equity HU vs a estimativa 27%), a margem "+13,2pp · com folga" (pot odds HU, oposta a "realiza 19% < 24%"), o selo "≈ Aproximação" (do solve HU capado) e a seção "Análise GTO · Processando" (porque zeramos o `gto_label`). Além de a fonte do veredito mostrar "Heurística" em vez de "Multiway". Análise do card inteiro (não remendo pontual): nova variante de fonte `multiway` (chip teal "Multiway") em `DecisionCard`/`DecisionCardV2`, e cada seção derivada do HU agora é ocultada quando há `multiway_advice` — restam só a estimativa multiway, o selo "Multiway · {n}-way" e a geometria (SPR/stack/M/ICM). Removida a duplicação de "Recomendado Fold". tsc ok, vitest 35/35.
