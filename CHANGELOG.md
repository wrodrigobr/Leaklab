...
- **Avaliação pela distribuição GTO (postflop)**: `/player/spots/drill/submit` (usado por Sparring **e** Ghost Table) avaliava só contra a ação **mais frequente** — marcava errado quem jogava uma ação **mista** legítima. Agora `_resolve_best_action_from_node` devolve as frequências do `strategy_json` e o grading aceita qualquer ação com freq ≥ 10% como correta (`mixed`), alinhando o veredito ao conceito `gto_mixed` do Replayer. Resposta ganha `gto_freq` e `mixed`. (O Replayer não foi afetado — o veredito dele já vinha do `gto_label` persistido, distribution-aware.)
