...
- **Frontend**: `AcademyGtoPreflop.tsx` (página com fluxo de submit→veredito + barra de frequências GTO), rota `/academy/gto-preflop`, tipos + client `gtoPreflop` em `api.ts`. Ativados os cards **"Ranges GTO Preflop"** (intermediate) e **"GTO Preflop — vs 3-Bet"** (advanced) no hub da Academia (antes eram *coming soon*). i18n nas 3 locales.
