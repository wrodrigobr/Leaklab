...
### test(coach-replay): corrige teste stale do /player/coach-replay e registra no CI (#tests)

> O teste `test_coach_replay` falhava com `KeyError: 'mistakes'` (falha pré-existente) e não estava em nenhuma suite do `run_all_tests.py`, então o CI não pegava. Causa: o contrato do endpoint mudou e o teste ficou defasado, não o contrário. O `build_coach_replay` hoje devolve a playlist sob `hands` (não `mistakes`) e cada item traz `narration` (não `coach_note`); por decisão de design a narração NÃO inclui o número de EV (o custo em bb vive no badge `ev_loss_bb`, pra evitar número divergente no texto). Ajustado o teste aos nomes atuais e, em vez de mascarar a ordenação, o seed agora faz a ordem cronológica DIVERGIR da ordem por custo (H1 mais barato porém 1º, H2 mais caro porém 2º), provando que o endpoint ordena por `hand_id` asc (se ordenasse por custo, `ms[0]` seria H2). Teste renomeado para `..._chronological_and_pro_gate`. Registrado `test_coach_replay.py` na suite `database` (junto dos demais `test_coach_*`). Verificado: 2/2 no arquivo, suite `database` 152/152, zero regressões. **Deploy:** nenhum (só testes).
