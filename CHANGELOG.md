...
### feat(leaktrainer): Sparring vira Treinador Adaptativo de Leaks (spots GTO canônicos)

> O Sparring deixou de **replayar mãos reais** (confuso: seguia uma linha fixa, spoilava a ação do herói, misturava "o que você jogou" com "o que o herói fez") e virou um **treinador adaptativo por leak**: identifica as categorias onde o jogador mais perde EV (`get_leak_categories`, severidade de `gto_label`) e apresenta **spots GTO canônicos** dessas categorias — cenários representativos limpos, **não** as mãos do jogador. O próximo spot **adapta-se à performance** (errou uma categoria → treina mais; sequência de acertos → sobe a dificuldade). 100% via dado pré-capturado (`analyze_preflop`) — **zero solve ao vivo** (AGPL). **Backend:** novo `leak_trainer.py` (`build_curriculum`/`generate_canonical_spot`/`grade_canonical_spot`/`next_spot`, generaliza o blueprint do `academy_gto_preflop`); endpoints `POST /player/leaktrainer/{next,grade}`; o `/next` **não revela a resposta** (só volta no `/grade`, range fica no servidor). **Frontend:** `Sparring.tsx` repurposed pro stream adaptativo — sem `tournaments.replay`/`gto.decisionLookup`/`decision_id`; `gtoData` sintético do grade alimenta o `CoachCard` (veredito 3-níveis + barras) sem mudá-lo; `session_state` por categoria no localStorage; header mostra a categoria treinada + progresso; copy i18n (3 locales). MVP **preflop** (rfi/vs_rfi/vs_3bet, cobertos pelas ranges); **postflop = Fase 2** (catálogo pré-solvado offline). Testes: `test_leak_trainer` 26; engine 391/391, api+database verdes; smoke end-to-end (next/grade/anti-leak/auth).
