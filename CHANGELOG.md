...
### fix(preflop): alargamento do range de shove em stack muito curto (<9bb) (#veredito)

> **Bug real de veredito em stack curto, diagnosticado com dados.** O bucket de ranges `10bb` é calibrado para ~9-10bb (UTG ~16%), mas cobre 0-12bb. Um jogador com 5-7bb reusava esse range apertado, então shoves **corretos** (A7o, K9o, KTo, QTo, J9s do UTG a 6bb) eram marcados como `major_leak`, punindo o aluno por jogar certo (o range correto a 6bb quase dobra, ~30%). **Nota:** a crítica antiga de sizing "2-2,5bb" em stack curto já estava corrigida (gate `_eff_stack > 12` em `app.py`), e rodar `add_pushfold_ranges.py` seria inócuo (o fallback `push_fold` só dispara sem RFI, e o bucket 10bb TEM RFI). O bug que sobrava era o range apertado demais. **Correção (heurística, escolha do usuário):** abaixo de ~9bb, um shove first-in fora do range estrito é RESGATADO para `acceptable` quando a equity da mão vs um range de call plausível cobre um piso que cai conforme o stack encurta (matemática de push/fold: menos fichas = mais fold equity). SÓ resgata (nunca cria leak novo): trash (72o/32o/K5o) segue `major_leak`, o range estrito de ≥9bb fica intacto, e o fold nunca é punido. Usa a matriz de equity 169×169 (`equity.equity_vs_range`). Selo novo no card "≈ push/fold curto (aproximação)" (`pushfold_widen` → Replayer + i18n 3 locales). `preflop_gto_ranges.py` (`_widen_floor`, `_shove_equity`, `_SHOVE_CALL_RANGE`). Teste novo `test_short_stack_shove_widening_no_false_leak`. Suites: engine 396, api 44, regressões 28, todas verdes. tsc + build OK. **Deploy:** frontend (Cloudflare) + `web`.
