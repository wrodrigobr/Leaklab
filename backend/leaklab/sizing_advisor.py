...
def gto_open_to_bb(position: str, stack_bb: Optional[float]) -> Optional[float]:
    ...
    if not position or not stack_bb:
        return None
    try:
        from leaklab.preflop_gto_ranges import _load, balde_rfi, raise_to_bb_from_node
        ...
        bucket = balde_rfi(float(stack_bb))   # RFI da própria profundidade na faixa 3-7bb
