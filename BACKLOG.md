...
| AY-23 | **Referencia de C-Bet IP/OOP pelo SOLVER nos proprios spots** (07/09; os 60-75 / 45-60 que entraram nao tinham fonte e sairam). MEDIDO no acervo do Rullian (copia em dev, conta `rullian_copia@dev.local`, id 992828; iguala prod): 1.184 spots heads-up, 1.037 com solve (755 IP, 281 OOP). Acao principal do solver = apostar em 73% IP / 14% OOP; mas so 25% dos spots guardam `gto_top_freq`, e nesse subconjunto a P(apostar) media e 70% IP / 27% OOP: a acao principal esconde a mistura, sobretudo OOP. Jogador: 90% IP / 68% OOP. Desenho proposto: referencia = media da frequencia de aposta do solver nos proprios spots (le a estrategia completa dos nos, nao a acao principal), com folga estatistica e cobertura, como o VPIP/PFR; exige backfill da frequencia nos 75% sem `gto_top_freq` (ler os nos em prod). | Rascunho antes de construir, como o dono pediu nas reguas anteriores. |
...
| AY-21 | **Opcao "agrupado" na grade por posicao: EP / MP / CO / BTN / SB / BB** (conversa com o Rullian, 07/09: o PT4 junta tudo antes do CO em EP e MP; "simplifica, mas fica ruim de estudar"; o dono pensou em um toggle detalhado x agrupado). `grupo_posicional` ja define EP = UTG/UTG+1/UTG+2 e MP = LJ/HJ. A regua do grupo = referencia ponderada pelas maos de cada assento do grupo. | Ideia registrada; decidir depois do deploy. |
