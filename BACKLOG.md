...
| AY-4 | **Eixo de tempo: 27 funcoes filtram por data de UPLOAD — e 24 delas nem aceitam o filtro da tela.** Consertado em 03/09 no `_build_tournament_filter` (19 chamadores); as outras 27 nunca adotaram. **Segunda camada, achada em 05/09 respondendo a uma hipotese do dono ("o novo padrao e o historico, entao ele nao percebe, certo?"): NAO.** Das 27, so 3 aceitam `last_n` (`get_evolution_metrics`, `get_gto_leak_ranking`, `get_leak_roi_impact`). **As outras 24 usam `days` fixo, sempre** — `get_strategic_twin_profile(days=180)`, `get_cognitive_failure_report(90)`, `get_leak_graph_data(90)`, `get_player_dna(90)`, `get_pressure_profile(90)`, `get_confidence_drift(30)`, `get_player_level(30)`, `get_career_projection`, `get_icm_performance(90)`, `get_breakdown(90)`, leaderboard, coach impact/common-leaks e mais. O filtro do dashboard **nao as alcanca**: escolher "Historico" ou "ultimos 20" nao muda nada nesses cards. Sao DOIS defeitos empilhados: (1) o eixo errado; (2) a tela promete um escopo que 24 cards nao entregam. O segundo ficou mais grave em 05/09, porque a faixa de escopo nova AFIRMA de largura inteira "estes numeros sao de todo o seu historico" — e isso e falso para 24 dos cards abaixo dela. | **Impacto medido (05/09, 180d):** micheldienstmann25 ve **13.878 decisoes pelo eixo de upload contra 1.318 pelo de jogo — 10,5x**; wrodrigo +18%; os demais, zero (importam conforme jogam ou todo o historico cabe na janela). Atinge exatamente quem chega novo e sobe acervo antigo. **Ao atacar, o trabalho e maior que trocar a coluna:** cada uma das 24 precisa passar a ACEITAR `last_n` e propaga-lo desde o endpoint (o frontend ja manda), e so entao usar `_build_tournament_filter` em vez de reescrever a regra. Classificar funcao a funcao em tres baldes — janela de ANALISE ("como voce joga"), PRESENCA de dados (o check do plano de estudos precisa de `imported_at`, nota em `get_evolution_metrics` ~1185) e uso ADMINISTRATIVO (`get_admin_dashboard_stats`, `get_public_coaches`, `count_user_pending_solves` provavelmente nem devem mudar). Converter so o primeiro balde. Fechar com teste que varra as N+1: nenhuma funcao de janela de analise usa `imported_at` cru nem ignora `last_n`. Medir antes/depois por usuario, michel em especial — o perfil dele vai mudar de verdade (1.318 em vez de 13.878) e cards podem cair em amostra baixa onde antes mostravam numero. E a leitura correta, mas e perceptivel. |
