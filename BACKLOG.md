...
| AY-26 | **Card acusa erro com `best_action` igual a jogada (varredura AUTO: 62 casos em prod, 08/09).** Decisoes de 03/09 a 07/09 (usuarios 62, 65, 40), pos-flop, `label` small/clear_mistake com `best_action == action_taken` e `gto_action` diferente (ex.: 379268 bet acusado de clear_mistake, best_action=bet, gto_action=check). Provavel: o reconcile pos-solve muda o label pelo `gto_label` sem atualizar `best_action`, ou o motor grava best_action=jogada e o solve chega depois. Mais 1 GRAFIA (364518 shove vs jam, marginal 0.18). Conferir com a lista de 'quem muda veredito carrega recomendacao e score junto' (v0.168). | Nao e regressao do deploy de 07/09; corrigir na proxima janela. |
