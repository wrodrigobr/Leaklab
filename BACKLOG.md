...
| Guard no engine para squeeze/3-bet "a frio" (durabilidade do fix) | `fix_preflop_3bet_misclass.py` corrige o DADO (gto→NULL), mas `analyze_preflop` ainda recomputa o gto errado para spots de 3-bet/squeeze enfrentados a frio (sem range vs_3bet apropriada) → tratados como vs_RFI. Sem o guard no engine, qualquer `reanalyze_all_labels` re-introduz o veredito errado (drift detectado: 4→43 após o patch). **Fix:** em `analyze_preflop`, quando `preflop_raises_faced>=2` e o hero não é o 3-bettor e não há range vs_3bet/squeeze → retornar no-coverage (available=False). Com tests. Pré-requisito antes de re-rodar reanalyze. |
