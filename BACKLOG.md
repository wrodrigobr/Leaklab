...
| AY-23 | **Referencia de C-Bet IP/OOP pelo SOLVER nos proprios spots** (07/09; o dono duvidou dos 60-75 / 45-60 que entraram sem fonte, e estava certo: o agressor IP c-beta muito no solver moderno). Plano: para cada oportunidade de c-bet com solve (`gto_label`/`gto_action` no flop), contar quantas vezes o solver recomendou apostar, IP e OOP; referencia = essa frequencia com folga estatistica e cobertura declarada, como o VPIP/PFR. Em dev nao ha solve pos-flop (0 de 308 oportunidades); medir em prod (Rullian) antes de desenhar. Ate la o tooltip mostra IP/OOP so com numero e amostra. | Depois do deploy; precisa de ssh para medir a cobertura. |
