...
Status: **COMPLETO 2026-07-01** (fases 1-6). Parser ACR no `leaklab/parser.py` (branch `site=='acr'`); validado nos 3 arquivos reais (86 mãos → 136 decisões com posição). **Fase 5 (financeiro) FECHADA:** o corpo ACR é só em chips → o buy-in vem do FILENAME (`TN-$0{FULLSTOP}50`→$0.50); o frontend agora manda o `filename` no `/analyze` (`tournaments.analyze(content, file.name)`), o backend parseia via `_acr_buyin_from_filename`, e o `_extract_financials` ACR **NÃO assume busted** (prize/profit ficam None = "resultado desconhecido", pra não inventar prejuízo — o HH ACR não traz resultado nem payout). Testes `tests/test_acr_parser.py` (6, suite regression). Arquivos de amostra em `backend/HH20260630 SITGOID-G35409697T*.txt` (MESMO torneio #35409697 — merge por hand_id no import). **Prize/ROI via arquivo de RESULTADOS ENTREGUE 2026-07-01 (#34):** o ACR tem um Tournament Summary separado (`.ots`, JSON: `tournament_number`, `prize_pool`, `player_count`, `tournament_finishes_and_winnings[]` com place+prize por jogador). `parser.parse_acr_results` parseia; endpoint `POST /tournament/results` casa por Tournament #, acha o hero (soma re-entradas, melhor colocação), pega buy-in TOTAL do filename do summary (`$0.50 + $0.05`=0.55) e atualiza prize/profit/place/buy_in (`update_tournament_financials`). Front: botão "Resultado" (âmbar) na coluna de profit dos torneios ACR sem resultado (`Tournaments.tsx`), upload `.ots` → banner de confirmação → reload. Validado com o arquivo real (MusashiBR 6º, prize $1.19, buy-in $0.55, profit +$0.64). Testes em `test_acr_parser.py`.
