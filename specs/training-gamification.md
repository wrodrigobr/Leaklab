...
- **A JORNADA (framing, ideia do dono 2026-06-30) — 3 etapas, com GATE de prontidão:**
  ...
  2. **Aplicar** — DESBLOQUEIA só quando o jogador atinge "nível bom" num leak treinado (proposta: tier **Ouro / mastery≥70** numa habilidade; tunável). Aí aparece o convite, mirado naquele leak: *"Você dominou BB defense no treino. Jogue um torneio focando nisso e importe."* O gate evita incomodar quem ainda não praticou e faz a sugestão ser **conquistada**. É uma SUGESTÃO, não um bloqueio (o jogador sempre pode jogar/importar).
  3. **Provar** — após o upload, compara o indicador real antes×depois (a Fase 4 abaixo).
  Implementável já na Etapa 2 sobre dados existentes (`overview.skills` mastery): mostrar a CTA "Aplicar" quando alguma skill treinada cruza o tier-alvo. UI: uma trilha de 3 etapas no hub de Treino + a CTA contextual.
